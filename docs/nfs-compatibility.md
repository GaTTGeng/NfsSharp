...
| Timeouts and retries | `NfsClientOptions` | **Partial** | M3 | Covered by unit tests for invalid command-timeout, retry-count, and retry-delay option rejection, plus repository NFS-Ganesha integration coverage for reconnecting and retrying after a disposed transport connection. Remaining work: deterministic fault injection, cancellation precedence, duplicate request safety, server restart behavior, and backoff policy. |
